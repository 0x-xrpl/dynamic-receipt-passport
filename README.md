...
Mint 結果は Testnet Explorer へのリンクとともに確認でき、パスポートビューには NFToken ID / Tx Hash / Wallet Address などの情報が反映されます
...
Add Purchase
XRPL 送金モードでは、TX Hash プレビュー、送信ロック、Live Status Row、Explorer への直接リンクなど XRPL デモに必要な UI を統合します。

Passport
Dynamic NFT パスポートビュー。成長の可視化を行います。
...
Map
購入スポットと XRPL 決済スポットを表示します。

Export
日付範囲を指定した集計・CSV 出力・印刷ビューを提供します。

Settings
通貨単位、言語、XRPL 接続設定など（MVP では一部のみ）を管理します。
...
AI による支出パターン分析やパーソナライズされた旅提案

9-1. Web2 レイヤーとしての Email / ポイント設計
DRP では、Email ログインのみで使い始められる Web2 体験 をとても重要なレイヤーとして位置づけています。

ウォレットを持っていないユーザーでも、「ふつうのポイントアプリ」と同じ感覚で使える

レシート登録やスタンプ、簡易的なダッシュボードまでは Email だけで完結

後からウォレットを接続することで、段階的に Web3 / XRPL の世界に入っていける

これにより、

既存の Web2 ユーザー（ポイントカードやスタンプアプリに慣れた層）を自然に取り込みつつ

「必要になったタイミングでウォレットをつなぐ」というソフトな導線

を提供することを目指しています。Web2 と Web3 の間に「段差を作らない」ことが、DRP の根底にある設計思想です。

9-2. XRP 還元・ホテル提携・XRP ファンコミュニティとの共創
将来的な構想として、DRP は ポイント構造と XRP 還元 を組み合わせたエコシステムを視野に入れています。

レシート登録やスタンプ達成に応じてポイントが貯まり、一部を XRP として還元する仕組み

提携ホテル・宿泊施設とのコラボレーションによる、「一定ランクのパスポート保持者向けの特典（宿泊割引・ラウンジ利用など）」

XRPL 決済対応店舗やローカルビジネスとのタイアップキャンペーン

ここで大切にしたいのは、一方向の「ポイント配布」ではなく、双方向のプロジェクトであるという点です。

一方の軸は、レシートやスタンプを通じて日常的に利用する「消費者」と「小売・飲食・ホテル・ローカルビジネス」

もう一方の軸は、XRP を世界中に広めたい「XRP ファン」「XRPL 愛好家」コミュニティ

DRP は、この二つの軸が交差する場所に 「Dynamic NFT パスポート」 を置き、

日常の支出や旅の記録が、そのまま XRP エコシステムへの参加になっていく

XRP 側のコミュニティからも、「自分たちの好きなチェーンを日常に溶け込ませていく」動きとして支えられる

そんな 双方向で紡がれる一大プロジェクト として発展していくことを目指しています。
...
といったケースごとに、ユーザーが状況を理解できるメッセージを表示し、ボタンが「押せなくなる」状態を避けるように実装しています。
