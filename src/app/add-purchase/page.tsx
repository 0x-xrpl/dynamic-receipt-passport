...
import { sendXrp } from "@/lib/legacyAdapter";
...
export default function AddPurchasePage() {
  ...
  const [destination, setDestination] = useState("rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe");
  ...
  const handleSendXrp = async () => {
    if (!form.amount) {
      setSendStatus("Enter an amount before sending XRP.");
      return;
    }
    setSending(true);
    setSendStatus("Connecting to XRPL…");
    try {
      const result = await sendXrp(destination, parseFloat(form.amount));
      const nextHash = result?.hash || generateHash();
      setHash(nextHash);
      setSendStatus(`Submitted · ${nextHash.slice(0, 10)}…`);
    } catch (err) {
      console.error(err);
      setSendStatus(err instanceof Error ? err.message : "XRPL send failed");
    } finally {
      setSending(false);
    }
  };
    ...
    <AppShell heroSubtitle="Smooth XRPL Testnet capture" contextLabel="Add purchase">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Section title="XRPL testnet preview">
          ...
          <Input placeholder="Destination XRPL wallet" value={destination} onChange={(e) => setDestination(e.target.value)} />
          <Button type="button" variant="outline" onClick={handleSendXrp} disabled={sending}>
            {sending ? "Sending…" : `Send ${form.amount || "0"} XRP`}
          </Button>
          {sendStatus && <p className="text-xs text-white/60">{sendStatus}</p>}
        </Section>
